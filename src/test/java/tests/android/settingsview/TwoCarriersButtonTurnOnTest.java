...
public class TwoCarriersButtonTurnOnTest extends BaseTest {
    ...
    @Test(invocationCount = 10) //Verify One carrier button can TurnOn and new Settings will Apply on the Main View
    public void verifyThatCarrierButtonTurnOn() {
        ...
        //Turn On Lyft button
        sv.carrierButtonTurnOn(sv.lyftButton);

        //Check attribute checked status
        assertEquals(sv.getCheckedStatusUiElement(sv.lyftButton), "true");

        //Navigate to the Main View
        sv.continueTap();

        //Open PopUP menu with Carriers
        ...
        assertEquals(popCM.getListOfCarriersFromPopUpMenu().get(1).getAttribute("text"), "Lyft");
    }
